...
essencial no dia-a-dia, até mesmo a populações fisicamente isoladas de qualquer região
urbana. Empresas bem conhecidas, como Vivo e Claro, vêm se empenhando para
...
O design deverá ser responsivo, podendo ser utilizado em plataformas \textit{mobile}
ou \textit{desktop}, e simples, com controles com experiência de usuário (UX) intuitiva
...
dos produtos encontrados, chamado Atoll, é um \textit{software} de planejamento de
...
vemos como há necessidade do mercado por uma ferramenta voltada à ampliação
...
antenas, já que há demanda dos usuários por um acesso à Internet melhor e
...
essa seção da monografia se dedicará a como o SIG permite a modelagem dos
...
em disco e os algoritmos de filtro de distância e pertencimento a uma área são em geral
mais rápidos.
...
$d$ da antena, a intensidade do sinal caí pelo inverso do quadrado. Podemos
...
\end{equation*}
A atenuação de sinal $R$ entre uma antena transmissora e uma antena receptora pode
ser escrita como:
\begin{equation*}
...
obter um ganho de desempenho no sistema. No trabalho em questão, os
...
associados, que correspondem cada parâmetro a valores distribuídos uniformemente
...
quantidade de níveis adequada. Um \textit{array} ortogonal é uma tabela na qual há um
número $t$ (chamado de força do \textit{array} ortogonal), sendo que, para qualquer seleção de
...
total das ERBs instaladas. As bibliotecas geoespaciais, como a GEOS, oferecem funções
para cálculo da união de figuras geométricas, de modo que as áreas de intersecção
...
Primeiramente, estudamos o ``Mapa de ERBs Brasil'', presente no portal Telebrasil~\cite{mapa-erb}.
Essa base contém uma lista de ERBs do Brasil de novembro de
2017 com informações de operadora, endereço, e posição geográfica de cada ERB.
Essas informações são essenciais no cálculo da posição ótima da ERB para
maximizar a cobertura da célula de cada operadora.
...
ao longo do ano para cada semana. Indicamos também o período de módulo de estágio em
...
obras de referência na literatura de otimização de antenas, e pesquisa de \textit{softwares}
com escopo parecido. A ideia inicial era ter como ator principal do sistema o funcionário
cadastrado de uma empresa de instalação de antenas, porém depois consideramos que seria
...
poderia ajudar esses atores, e quais informações ele poderia produzir. Esse processo
nos levou à modelagem de diversos casos de uso de interesse do sistema. Alguns
foram deixados de lado por fugirem do escopo do MVP, e estão listados na seção
...
soluções encontradas foram mencionadas abaixo.
...
referente a uma de duas bases de dados. Uma das base utiliza
dados oficiais da Anatel para indicar as antenas oficialmente registradas que,
portanto, pertencem a uma única operadora oficialmente~\cite{mapa-erb}.
...
banco de dados, equivalente ao número de operadoras do compartilhamento~\cite{opencellid}.
Como possuem um identificador de padrão global, estes dados foram chamados de ``Antenas
...
suporte nativo do Django ao desenvolvimento para linha de comando, para facilitar o
uso.
...
relação, já que o seu \texttt{mnc} e o \texttt{valor} do outro modelo serão iguais.
Isso permite que o CGI dessas antenas
seja obtido apenas por seus campos, sem requisitar o respectivo MNC na
...
possível. Porém, mais antenas do que isso causava vários problemas relacionados
...
para tentar amenizar estes efeitos, como otimizar scripts; mas não houve vantagens perceptíveis.
...
para uma aplicação de visualização em mapa com vários clientes.
...
fórmulas do PostGIS específicas para clusterização de pontos. Para o armazenamento temporário dos
\textit{clusters}, utilizou-se o cache padrão do Django. Os resultados gerados pelo programa
...
da máquina do servidor, mas não houve ganhos consideráveis. Além disso, os dados
de \textit{cluster} ficam com aparência estranha, onde a separação em grade fica muito
...
visualização do mapa, embora consideravelmente melhores no geral; mas combinado com o uso de
...
e às vezes muito longe do centro, deixando muitas áreas vazias.
...
metros no Equador, o que é o suficiente para os níveis de zoom utilizados.
Além disso, o código do \textit{front-end} foi modificado especificamente para fins de
desempenho, assim obtendo clusterizações com visualização mais natural.
Graças ao suporte do GeoDjango para inserir o \textit{geohash} em \textit{queries}, foi possível
realizar a implementação em um algoritmo simples, sem escrever as \textit{queries}
...
Por fim, foi implementada a lógica adicional ao adicionar novas antenas no banco de dados, para
atualizar automaticamente os respectivos \textit{clusters}.
...
no capítulo \textbf{``Aspectos Conceituais''}. Os \textit{arrays} ortogonais usados pelo
...
Além disso, tivemos um entendimento melhor de seu funcionamento uma vez que nós
o implementamos. Os detalhes sobre os testes mencionados
encontram-se no capítulo ``Testes e Avaliação''.

\subsection{Interface de Programação de Aplicações}

A integração do \textit{back-end} com o \textit{front-end} foi feita por uma API HTTP, que
abstrai as funcionalidades e os dados necessários para a realização dos casos de
uso. Resumidamente, essa API deverá ser responsável por três tarefas específicas:
...
novamente a cada execução. O uso desses dados segue os passos descritos para
dados \textit{raster} do capítulo ``Aspectos Conceituais''.
...
permitir o acesso a todos. Nesta máquina, instalamos o repositório em Python, o banco
...
Por fim, para facilitar o \textit{deployment} da aplicação para testes, configurou-se
...
necessidade de seu uso para testes. Os dados de acesso para serviços sociais também
foram tornados opcionais pela mesma razão.
...
comunicação do cliente com a API do Django, pela exibição dos dados no mapa,
...
botão para executar o algoritmo de cálculo da posição ideal a partir de parâmetros fornecidos
pelo usuário (figura \ref{fig:rascunho_mapa}). Criamos os desenhos com papel e lápis,
visto que tratam-se de uma representação de baixa fidelidade.
...
O objetivo dessa tela é fornecer uma ideia inicial das funcionalidades do
nosso projeto.
...
componentes pequenos e reusáveis. Outro benefício é o \textit{single-page application} que
o Vue.js disponibiliza. Com ele, a página não é recarregada quando o usuário muda de tela,
mas apenas os componentes diferentes entre telas que são trocados. Dessa forma, os
componentes comuns a várias telas (como \textit{header} e \textit{footer})
...
Com isso, a tela final (figura \ref{fig:tela_mapas}) ficou um pouco diferente da
...
A tela de \textit{login} (figura \ref{fig:tela_login}) é um formulário simples com um campo de usuário e senha.
...
O \textit{front-end} foi responsável por fazer as chamadas da API.
Usamos o \texttt{axios.js}, uma biblioteca JavaScript bastante popular, usada para fazer
...
para armazenar esse \textit{token} no escopo global. O Vuex permite que todos os
componentes da aplicação compartilhem o mesmo estado e que a mudança de estado
seja previsível e refletida para todos os componentes.

Com esse \textit{token} em mãos, podemos fazer as chamadas de API restritas aos usuários
...
possui suporte a HTTPS por padrão, e permite configurar as entradas do DNS facilmente.
...
diferentes implementações, dependendo do caso de teste. Os resultados obtidos foram
descritos a seguir.
...
Selecionamos uma área retangular e a implementação com Método Taguchi, e rodamos
...
resultados das figuras \ref{fig:basinhopping_exemplo} e
...
conseguimos obter resultados muito satisfatórios neste quesito, que atendesse às
expectativas de qualidade do grupo.
...
rápidos e mais precisos, buscando-se alternativas à solução de otimização
atual.

Também conseguimos expandir nossos conhecimentos sobre aplicações web
acerca do design, desenvolvimento e \textit{deployment}. Ainda melhoramos nossa
...
otimização para uma análise mais detalhada, ou de UX para verificar a
facilidade de uso da interface, mas não puderam ser efetuadas pela mesma restrição
de tempo. De qualquer forma, estes serão mais interessantes no futuro
...
empresas fundadas por alunos~\cite{neu}.
...
que seriam mais interessantes a uma empresa de telecomunicação em termos de
análise de risco.

\subsection{Descrição dos Casos de Uso Adicionais}

% Aviso para Bruno Albertini
% Nós não nos decidimos se é melhor tirar ou deixar esta subseção na monografia
% pois já tínhamos bolado esses casos de uso bem atrás.
% Outra possibilidade que pensamos foi simplificar esta parte e só falar por
% cima sobre os casos de uso pensados.
